...
const mldsaSupported = (() => {
  try {
    if (!eddsaSupported) return false;
    const keys = crypto.generateKeyPairSync('ml-dsa-65');
    const data = Buffer.from('a');
    const sig = crypto.sign(null, data, keys.privateKey);
    return crypto.verify(null, data, keys.publicKey, sig) === true;
  } catch {
    return false;
  }
})();
...
if (mldsaSupported)
  DEFAULT_SERVER_HOST_KEY.unshift('ssh-ed25519-ml-dsa-65');
