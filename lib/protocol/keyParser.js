...
const { eddsaSupported, mldsaSupported, SUPPORTED_CIPHER } = require('./constants.js');
const {
  bufferSlice,
  makeBufferParser,
  readString,
...
const SYM_ED_PRIV_KEY        = Symbol('Ed25519 Private KeyObject');
const SYM_MLDSA_PRIV_KEY     = Symbol('ML-DSA-65 Private KeyObject');
const SYM_ED_PUB_KEY         = Symbol('Ed25519 Public KeyObject');
const SYM_MLDSA_PUB_KEY      = Symbol('ML-DSA-65 Public KeyObject');
const SYM_HYBRID_PUBKEY_BLOB = Symbol('Hybrid raw pubkey blob');
...
// SPKI DER prefix for ML-DSA-65 public keys (22 bytes).
// Captured once at startup; used in parseDER to reconstruct KeyObjects from
// raw public key bytes received on the wire.
let MLDSA65_SPKI_PREFIX;
if (mldsaSupported) {
  const _testMLDSAKeys = require('crypto').generateKeyPairSync('ml-dsa-65');
  const _spki = _testMLDSAKeys.publicKey.export({ type: 'spki', format: 'der' });
  MLDSA65_SPKI_PREFIX = _spki.slice(0, _spki.length - 1952);
}

// Returns the SSH wire-format public key blob for ssh-ed25519-ml-dsa-65.
function genOpenSSHHybridPub(edpub, mldsapub) {
  const algoName = 'ssh-ed25519-ml-dsa-65';
  const algoLen = algoName.length;            // 21
  const keyBlob = Buffer.concat([edpub, mldsapub]); // 1984 bytes
  const buf = Buffer.allocUnsafe(4 + algoLen + 4 + keyBlob.length);
  let p = 0;
  writeUInt32BE(buf, algoLen, p);
  buf.utf8Write(algoName, p += 4, algoLen);
  writeUInt32BE(buf, keyBlob.length, p += algoLen);
  buf.set(keyBlob, p += 4);
  return buf;
}

// Internal factory: creates a HybridKey plain object and assigns its prototype.
function makeHybridKey({ edPubKey, mlPubKey, pubSSH, keyBlob,
                         edPrivKey = null, mlPrivKey = null }) {
  const obj = { type: 'ssh-ed25519-ml-dsa-65', comment: '' };
  obj[SYM_PRIV_PEM]          = null;
  obj[SYM_PUB_PEM]           = null;
  obj[SYM_PUB_SSH]           = pubSSH;
  obj[SYM_HASH_ALGO]         = null;
  obj[SYM_DECRYPTED]         = false;
  obj[SYM_ED_PUB_KEY]        = edPubKey;
  obj[SYM_MLDSA_PUB_KEY]     = mlPubKey;
  obj[SYM_ED_PRIV_KEY]       = edPrivKey;
  obj[SYM_MLDSA_PRIV_KEY]    = mlPrivKey;
  obj[SYM_HYBRID_PUBKEY_BLOB] = keyBlob;
  Object.setPrototypeOf(obj, HybridKey);
  return obj;
}

// Prototype for ssh-ed25519-ml-dsa-65 keys.
// Implements the signing and verification procedure from
// draft-josefsson-ssh-ed25519mldsa65.
const HybridKey = {
  sign(data) {
    const edPriv = this[SYM_ED_PRIV_KEY];
    const mlPriv = this[SYM_MLDSA_PRIV_KEY];
    if (!edPriv || !mlPriv)
      return new Error('No private key available');

    try {
      const pubkeyBlob = this[SYM_HYBRID_PUBKEY_BLOB];
      const r = randomBytes(16);
      const hPub = createHash('sha3-256').update(pubkeyBlob).digest();
      const h = createHash('sha3-256')
        .update(r)
        .update(hPub)
        .update(Buffer.from('Ed25519MLDSA65'))
        .update(Buffer.from('SSH'))
        .update(Buffer.from('SSH-Ed25519MLDSA65'))
        .update(data)
        .digest();

      const s1 = sign_(null, Buffer.concat([r, h]), edPriv);
      const s2 = sign_(
        null,
        Buffer.concat([s1, r, h]),
        { key: mlPriv, context: Buffer.from('ML-DSA-65-Ed25519-SSH') }
      );
      return Buffer.concat([s2, s1, r, h]);
    } catch (ex) {
      return ex;
    }
  },
  verify(data, signature) {
    const edPub = this[SYM_ED_PUB_KEY];
    const mlPub = this[SYM_MLDSA_PUB_KEY];
    if (!edPub || !mlPub)
      return new Error('No public key available');

    if (!Buffer.isBuffer(signature) || signature.length !== 3421)
      return false;

    try {
      const s2 = signature.slice(0, 3309);
      const s1 = signature.slice(3309, 3373);
      const r  = signature.slice(3373, 3389);
      const h  = signature.slice(3389, 3421);

      const pubkeyBlob = this[SYM_HYBRID_PUBKEY_BLOB];
      const hPub = createHash('sha3-256').update(pubkeyBlob).digest();
      const hExpected = createHash('sha3-256')
        .update(r)
        .update(hPub)
        .update(Buffer.from('Ed25519MLDSA65'))
        .update(Buffer.from('SSH'))
      ...
      if (!h.equals(hExpected)) return false;

      const rh = Buffer.concat([r, h]);
      if (!verify_(null, rh, edPub, s1)) return false;

      return verify_(
        null,
        Buffer.concat([s1, r, h]),
        { key: mlPub, context: Buffer.from('ML-DSA-65-Ed25519-SSH') },
        s2
      );
    } catch (ex) {
      return ex;
  ...
  isPrivateKey() {
    return this[SYM_ED_PRIV_KEY] !== null;
  },
  getPrivatePEM() { return null; },
  getPublicPEM()  { return null; },
  getPublicSSH()  { return this[SYM_PUB_SSH]; },
  equals(key) {
...
function parseDER(data, baseType, comment, fullType) {
  ...
  switch (baseType) {
    ...
    case 'ssh-ed25519-ml-dsa-65': {
      if (!mldsaSupported)
        return new Error('Unsupported OpenSSH public key type: ssh-ed25519-ml-dsa-65');
      const keyBlob = readString(data, data._pos || 0);
      if (keyBlob === undefined || keyBlob.length !== 1984)
        return new Error('Malformed hybrid public key');
      const edpub    = keyBlob.slice(0, 32);
      const mldsapub = keyBlob.slice(32);  // 1952 bytes
      const edPubKey  = createPublicKey(genOpenSSLEdPub(edpub));
      const mlPubKey  = createPublicKey({
        key: Buffer.concat([MLDSA65_SPKI_PREFIX, mldsapub]),
        format: 'der',
        type: 'spki',
      });
      return makeHybridKey({
        edPubKey,
        mlPubKey,
        pubSSH: genOpenSSHHybridPub(edpub, mldsapub),
        keyBlob,
      });
    }
    default:
...
// Creates a ssh-ed25519-ml-dsa-65 hybrid key from two Node.js KeyPair objects.
// Pass the output of crypto.generateKeyPairSync('ed25519') and
// crypto.generateKeyPairSync('ml-dsa-65').  Include privateKey on both pairs
// for a signing key; omit (or pass null) for a public-only key.
function createHybridKey(edKeyPair, mldsaKeyPair) {
  const edSpki   = edKeyPair.publicKey.export({ type: 'spki', format: 'der' });
  const edpub    = edSpki.slice(-32);
  const mlSpki   = mldsaKeyPair.publicKey.export({ type: 'spki', format: 'der' });
  const mldsapub = mlSpki.slice(-1952);
  return makeHybridKey({
    edPubKey:  edKeyPair.publicKey,
    mlPubKey:  mldsaKeyPair.publicKey,
    edPrivKey: edKeyPair.privateKey  || null,
    mlPrivKey: mldsaKeyPair.privateKey || null,
    pubSSH:    genOpenSSHHybridPub(edpub, mldsapub),
    keyBlob:   Buffer.concat([edpub, mldsapub]),
  });
}
